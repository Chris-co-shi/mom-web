...
<template>
  ...
  >
    <template #brand>
      ...
      <div class="mom-auth-shell__brand-title">供应商协同门户</div>
      ...
      <Card class="auth-card" :bordered="false">
        <template v-if="runtimeState.phase === 'anonymous'">
          ...
          <Form layout="vertical" @finish="submitLogin">
            <FormItem html-for="supplier-login-username" label="用户名" required>
              <Input id="supplier-login-username" v-model:value="form.username" autocomplete="username" :maxlength="120" />
            </FormItem>
            <FormItem html-for="supplier-login-password" label="密码" required>
              <InputPassword id="supplier-login-password" v-model:value="form.password" autocomplete="current-password" :maxlength="128" />
            </FormItem>
            <Button
              block
        ...
        <template v-else>
          ...
          <Form layout="vertical" @finish="submitPasswordChange">
            <FormItem html-for="supplier-change-username" label="账号">
              <Input id="supplier-change-username" :value="form.username" disabled />
            </FormItem>
            <FormItem html-for="supplier-new-password" label="新密码" required>
              <InputPassword id="supplier-new-password" v-model:value="form.newPassword" autocomplete="new-password" :maxlength="128" />
            </FormItem>
            <FormItem html-for="supplier-confirm-password" label="确认新密码" required>
              <InputPassword id="supplier-confirm-password" v-model:value="form.confirmation" autocomplete="new-password" :maxlength="128" />
            </FormItem>
