...
const entries = portalFeatures('customer');
</script>

<template>
  <a-layout class="app-shell">
    <a-layout-header class="app-header">
      <div><strong>客户协同门户</strong><span class="app-subtitle">Phase 02 Ready</span></div>
      <a-space>
        ...
        <template v-if="runtimeState.user">
          ...
          <a-tag color="purple">固定 Customer · {{ runtimeState.user.partyId }}</a-tag>
    ...
    <a-layout-content class="app-content">
      ...
      <div v-else-if="runtimeState.user" class="portal-page">
        <div class="hero"><div><a-tag color="purple">CUSTOMER PORTAL</a-tag><h1>客户协同门户</h1><p>当前主体由 IAM 固定绑定；页面不提供 Customer ID 输入、选择或切换。</p></div><a-button @click="refreshAccess">刷新授权上下文</a-button></div>
