...
<template>
  <div v-if="runtimeState.phase === 'starting'" class="auth-loading">
    <a-spin size="large" tip="正在校验客户会话" />
  </div>
  <main v-else-if="runtimeState.phase === 'anonymous' || runtimeState.phase === 'password-change'" class="auth-page">
    <section class="auth-brand">
      <a-tag color="cyan">CUSTOMER PORTAL</a-tag>
      <h1>客户协同门户</h1>
      <p>使用客户账号登录。账号只能访问 IAM 固定绑定的 Customer 与 Factory Scope。</p>
    </section>
    <a-card class="auth-card" :bordered="false">
      ...
      <template v-else>
        <h2>首次修改密码</h2>
        <p>完成改密后才会创建客户 Portal Session。</p>
...
<style scoped>
.auth-loading{min-height:100vh;display:grid;place-items:center;background:#f5f7fb}.auth-page{min-height:100vh;display:grid;grid-template-columns:minmax(18rem,32rem) minmax(20rem,27rem);gap:4rem;align-items:center;justify-content:center;padding:3rem;background:linear-gradient(135deg,#e6fffb,#f7f8fc 48%,#eef2f8)}.auth-brand h1{font-size:2.5rem;margin:.8rem 0}.auth-brand p{color:#5f6d80;line-height:1.8}.auth-card{box-shadow:0 24px 70px rgba(15,80,80,.14);border-radius:16px}.auth-alert{margin:1rem 0}@media(max-width:800px){.auth-page{grid-template-columns:1fr;padding:1.25rem;gap:1rem}}
</style>
